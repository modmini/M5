...
helpMessage =""" 👿  SAI Bot Lv.6.1.2  BotTag  BotProt 👿


👿☆ ☞ あ รายชื่อคำสั่งแต่ละคำ+รายละเอียดあ ☜☆👿


👿 sai help  👹คำสั่งเพื่อเรียกดูรายการคำสั่งทั้งหมด
👿 แท็กชื่อ   👹  คำสั่งแท็คชื่อคนในห้องแบบทั้งหมด เฉพาะคนในห้อง1-500คน
...
👿 ทรายแท็กชื่อ   👹  คำสั่งแท็คชื่อเฉพาะคนในห้องที่มี150คนลง
👿 ทรายแท็ก     👹คำสั่งแท็คชื่อเฉพาะคนในห้องที่มี150คนลง
👿 Me     👹คำสั่ง สั่งให้บอทส่งคอนแทคหรือข้อมูลติดต่อเราในห้อง
👿 ยกเลิก.    👹 ยกเลิกค้างเชิญทั้งหมด
👿 ไอดีกลุ่ม   👹  เช็คไอดีกลุ่ม
👿 sai mid     👹เช็คMid
👿 saiเช็ค    👹 รีเซ็ตระบบ บอทอ่าน
👿 saiอ่าน    👹 ดูคนแอบอ่าน
...
👿🔝 BOT แท็กชื่อ ราคา300บาท จ่ายที่เดียวจ👿บ 🔝
สนใจติดตั้งBOT ป้องกันวี10  BOTป้องกันต่างๆ  BOTแท็คชื่อทั้งห้อง  BOTกันคนลบ+ดูคนแอบอ่านแบบแท็กชื่อคนแอบอ่าน
บอทป้องกันแบบชุดมีตามนี้ ชุด 3+1 4+1 5+1 6+1 10+1 15+1 20+1 25+1 30+1 32+1 40+1 ระบบป้องกันก็ดีพออสมควร ลบ1ลบ ยกเลิกค้างเชิญบอทเตะ กันเปิดลิ้งค์ กันเชิญ และระบบอีกเยอะ
...
def bot(op):
    try:
		...
		for ticket_id in n_links:
			if wait["atjointicket"] == True:
              ...
              if msg.from_ in admin:
            ...
            elif msg.text in ["sai Set"]:
                md = "ระบบเช็คตั้งค่าการล็อค&ปลดล็อคการป้องกัน\n\n"
                ...
                if wait["contact"] == True: md+="􂤁􀅣>􏿿 Contact    : on\n"
                else: md+=" Contact    : off\n"
                if wait["autoJoin"] == True: md+=" Auto join : on\n"
                else: md +=" Auto join : off\n"
                if wait["autoCancel"]["on"] == True:md+="􂤁􀅣>􏿿￼ Group cancel :" + str(wait["autoCancel"]["members"]) + "\n"
...
#-----------------------------------------------
    ...
    #-------------Fungsi Tag All Finish---------------#
       ...
       #--------------Fungsi Broadcast Finish-----------#
            ...
            elif msg.text in ["สวัสดี","สวัสดีค่ะ","สวัสดีครับ","สวัสดีจ้า","สวัสดีคับ","สวัสดีจร้า"]:
                ki.sendText(msg.to,"สวัสดี")
                kk.sendText(msg.to,"ยิดดีตอนรับ")
                kc.sendText(msg.to,"")
...
#-----------------------------------------------
      ...
      #-------------Fungsi Balesan Respon Start---------------------#
		...
		ki.sendText(msg.to,"￼￼￼Bot V.10 12ตัว ราคา180บ.")
		ki.sendText(msg.to,"Bot V.10 37ตัว ราคา650บ.")
		ki.sendText(msg.to,"Bot V.10 ชุดบอทเสริม ราคาชุดละ100บ. 1.ชุดมี5ตัว 1ห้องสามารถเพิ่มได้มากสุด5ชุด")
		ki.sendText(msg.to,"Bot แท็คชื่อคนทั้งห้อง ราคา 300บ. ไม่มีรายเดือน จ่ายรอบเดียวจบ")
		ki.sendText(msg.to,"สนใจBot ติดต่อ ￼ ทราย 0902853778  http://line.me/ti/p/~bot_botv13 ￼ ￼ดาว 0908927220 http://line.me/ti/p/~devilgirl.￼")
